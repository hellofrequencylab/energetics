/**
 * Pythagorean numerology: the Life Path number as a medallion. The number is the
 * spine of the reading, so it is shown large and plainly. Master numbers (11, 22,
 * 33) are noted, since they are not reduced further.
 */
const MASTERS = new Set([11, 22, 33]);

export function LifePath({ value }: { value: number }) {
  const master = MASTERS.has(value);
  return (
    <svg width={150} height={150} viewBox="0 0 150 150" role="img" aria-label={`Life Path ${value}`}>
