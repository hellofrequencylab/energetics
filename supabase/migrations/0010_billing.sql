...
create table if not exists onesky.customers (
  user_id uuid primary key references auth.users on delete cascade,
  stripe_customer_id text unique,
  created_at timestamptz not null default now()
);
alter table onesky.customers enable row level security;
-- No policies: unreachable except via the service role.
grant all on onesky.customers to service_role;

-- --- Subscriptions (a faithful projection of the Stripe object) -------------
create table if not exists onesky.subscriptions (
...
alter table onesky.subscriptions enable row level security;
...
grant select on onesky.subscriptions to authenticated;
grant all on onesky.subscriptions to service_role;

create index if not exists subscriptions_user_idx on onesky.subscriptions (user_id);
...
-- and never recorded).
create table if not exists onesky.ai_usage (
...
alter table onesky.ai_usage enable row level security;
...
grant select on onesky.ai_usage to authenticated;
grant all on onesky.ai_usage to service_role;

create index if not exists ai_usage_user_day_idx on onesky.ai_usage (user_id, day);
create index if not exists ai_usage_ip_day_idx on onesky.ai_usage (ip_hash, day);
create index if not exists ai_usage_day_idx on onesky.ai_usage (day);
create index if not exists ai_usage_user_feature_idx on onesky.ai_usage (user_id, feature);
...
create or replace function onesky.is_plus(uid uuid)
returns boolean
language sql
stable
security definer
set search_path = onesky, pg_catalog
as $$
  select exists (
    select 1 from onesky.subscriptions
    where user_id = uid and status in ('trialing', 'active')
  ) or exists (
    select 1 from onesky.profiles
    where user_id = uid and is_admin
  );
$$;

revoke all on function onesky.is_plus(uuid) from public;
grant execute on function onesky.is_plus(uuid) to authenticated, service_role;
...
create or replace function onesky.enforce_chart_limit()
returns trigger
language plpgsql
security definer
set search_path = onesky, pg_catalog
as $$
begin
  if not onesky.is_plus(new.user_id) then
    if (
      select count(*) from onesky.birth_events
...
drop trigger if exists enforce_chart_limit on onesky.birth_events;
create trigger enforce_chart_limit
  before insert on onesky.birth_events
  for each row execute function onesky.enforce_chart_limit();
